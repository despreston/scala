...
object Anagrams {
   ...
   *
   */
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] = dictionary.groupBy(o => wordOccurrences(o))
  ...
  (occurrences foldRight List[Occurrences](Nil)) {
    case ((x, y), acc) => acc ++ (for {comb <- acc; n <- 1 to y} yield (x, n) :: comb)
  }
