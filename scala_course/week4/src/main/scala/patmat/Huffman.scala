...
object Huffman {
   ...
   */
    def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
      def findLeaf(workingTree: CodeTree, bits: List[Bit]): List[Char] = workingTree match {
        case Leaf(c,_) => if (bits.isEmpty) List(c) else c :: decode(tree, bits)
        case Fork(l, r, c, _) => if (bits.head == 0) findLeaf(l, bits.tail) else findLeaf(r, bits.tail)
      }

      findLeaf(tree, bits)
    }

  def testFrenchDecode(): List[Char] = {
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      var code = List[Bit]()
      def traverse(workingTree: CodeTree, c: Char): Unit = workingTree match {
        case Leaf(_,_) =>
        case Fork(l,r,ch,_) => {
